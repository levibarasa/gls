<head>
    ...
    <script type="text/javascript">
        ...
        if (${resmsuc == 'true'}) {
            alert("Workclass Resources maintained Successfully");
        }
        if (${resmodsuc == 'true'}) {
            alert("Workclass Resources Modified Successfully");
        }
        if (${resversuc == 'true'}) {
            alert("Workclass Resources Verified Successfully");
        }
        if (${noresmod == 'true'}) {
            alert("Workclass Has No resources for Modification");
        }
        if (${noresver == 'true'}) {
            alert("Workclass Has No resources for Verification");
        }
        if (${respendver == 'true'}) {
            alert("There is Pending Verification for the same Transaction\nCannot be Modified by this User");
        }
        if (${modversame == 'true'}) {
            alert("Modification and Verification User cannot be the same!");
        }
    </script>
</head>
...
<form id="form1" name="form1" method="post" action="do?MOD=BOK&ACT=dogomrp" onsubmit=" return validatePasswords(this)">
    <table width="70%" border="0" align="center" cellpadding="5" cellspacing="2">
        <tr>
            <th colspan="5" scope="col"><div class="header">&nbsp;Work Class Role Mapping</div></th>
        </tr>
        <tr>
            <td width="20%">&nbsp;</td>
            <td width="27%">&nbsp;</td>
        ...
        <tr>
            ...
            <td><label>
                    <input name="workclass" type="text" id="workclass" readonly="true" onkeyup="caps(this)" class="textboxes" />
                </label></td>
            <td><a href="" onclick="return getValue()"><img src="images/search.png"></a></td>
        </tr>
        <tr>
            <td>&nbsp;</td>
            <td>Work Class</td>
            <td style="color: red">*</td>
            <td><label>
                    <input name="workc" type="text" id="workc" readonly="true" onkeyup="caps(this)" class="textboxes" />
                </label></td>
        </tr>
        <tr>
            <td>&nbsp;</td>
